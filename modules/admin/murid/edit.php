...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif ($nis != $murid['nis']) {
        // Cek NIS sudah ada (hanya jika berubah)
        $stmt = $pdo->prepare("SELECT * FROM murid WHERE nis = ? AND murid_id != ?");
        $stmt->execute([$nis, $id]);
        if ($stmt->fetch()) {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("UPDATE murid SET 
                                  nis = ?, 
                                  nama_lengkap = ?, 
                                  jenis_kelamin = ?, 
                                  tanggal_lahir = ?, 
            ...
            $_SESSION['success'] = "Data murid berhasil diperbarui!";
            header('Location: index.php');
            exit;
        } catch (PDOException $e) {
            $errors['database'] = 'Gagal memperbarui murid: ' . $e->getMessage();
        }
    }
} else {
                    ...
                    <form method="POST">
                        <div class="card">
                            <div class="card-header bg-warning">
                                <h5 class="mb-0">Form Edit Data Murid</h5>
                            </div>
                            <div class="card-body">
                                <div class="mb-3 mt-3">
